...
class TelegramHTMLParser:
    def __init__(self) -> None:
        ...
        self.html_parser.protect_links = (
            True  # Protect hyperlinks from being stripped out
        )
    ...
    def parse_datetime(self, html_of_post: BeautifulSoup) -> str:
        date_time = self._html_to_text(
            str(
                html_of_post.find("span", {"class": "tgme_widget_message_meta"}).find(
                    "time", {"class": "datetime"}
                )
            )
        )
    ...
    def parse_images(html_of_post: BeautifulSoup) -> list[str]:
        images_raw_urls = html_of_post.findAll(
