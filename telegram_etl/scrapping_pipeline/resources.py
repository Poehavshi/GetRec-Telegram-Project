...
class LocalHackerNewsFetcher(APIFetcher):
    def __init__(self, path=None) -> None:
        self.path = (
            Path(path)
            if path
            else Path("data") / "dag" / "cache" / "cached_raw.snappy.parquet"
        )

    def fetch_content(self, context) -> pd.DataFrame:
        context.log.info(
            "Loading from file instead of API! For debugging purposes only!"
        )
...
@resource(
    config_schema={
        "endpoint": Field(
            str,
            is_required=False,
            default_value="https://hacker-news.firebaseio.com/v0/",
        ),
    }
)
...
@resource(config_schema={"path": Field(str, is_required=False, default_value="")})
