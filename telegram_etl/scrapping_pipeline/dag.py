...
@op(
    required_resource_keys={"hacker_news"},
    out={
        "items": Out(pd.DataFrame, is_required=False),
        "empty": Out(Nothing, is_required=False),
    },
)
...
def _post_to_slack(context, data=None):
    data = data or {
        "text": f"No new Hacker News found for {date.today().strftime('%b %d, %Y')}"
    }
    slack_hook = os.environ.get("SLACK_HOOK")
    if slack_hook is None:
        context.log.warning(
            "Environment variable **`SLACK_HOOK`** not specified. Logging to console only!"
        )
    ...
    def _build_slack_message():
        ...
        for index, row in df.iterrows():
            ...
            title_link = (
                row["url"]
                if row["url"] != "nan"
                else f"https://news.ycombinator.com/item?id={row['id']}"
            )
            clickable_title = f"<{title_link}|{row['title']}>"

            items.append(
                f"{bullet_point}\t{clickable_title} -- {row['by']} ({row['score']} points)"
            )
