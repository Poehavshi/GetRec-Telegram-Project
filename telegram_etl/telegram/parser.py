    ...
    async def run(self):
            ...
            match (
                ...
                case "3":
                    ...
                    file_path = os.path.join(
                        main_folder,
                        f"{main_folder}-{post_id}",
                        f"{main_folder}-Post-Content.txt",
                    )
                ...
                case "4":
                    ...
                    file_path = os.path.join(
                        main_folder,
                        f"{main_folder}-{post_id}",
                        f"{main_folder}-Bulk-Content.txt",
                    )
                    os.makedirs(os.path.dirname(file_path), exist_ok=True)
                    with open(file_path, "w", encoding="utf-8") as file:
            ...
            def download(file_type):
                ...
                for file_num, url in enumerate(array_to_watch, start=1):
                    file_path = os.path.join(
                        main_folder,
                        f"{main_folder}-{post_id}",
                        f"{file_type}-{self.dateTime}-{file_num}{file_extension}",
                    )
        ...
        try:
            for link in self.urlList:
                print(link)
                linkReq = requests.get(url=link, headers=self.headers)
                ...
                self.author = self.html_to_text(
                    str(
                        linkHTML.find("div", {"class": "tgme_widget_message_author accent_color"})
                ...
                self.dateTime = self.html_to_text(
                    str(
                        linkHTML.find("span", {"class": "tgme_widget_message_meta"}).find("time", {"class": "datetime"})
                    )
                )
